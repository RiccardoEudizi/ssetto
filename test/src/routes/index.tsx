    ...
    <main class="text-center  flex flex-col justify-center items-center  h-screen  text-black p-4 antialiased">
      ...
      <div class="max-w-5xl">
        <Drawer.Root
          side="bottom"
          contextId="xxx"
          bgSelector={document.querySelector("#app") as HTMLElement}
        >
          <div class="p-4 ">
            ...
            <div class="flex gap-x-12 mt-8">
              <Drawer.Trigger
                contextId="xxx"
                class="rounded-xl bg-stone-50 border border-stone-300 shadow-sm border-opacity-75  px-4 w-fit self-center py-3 text-lg font-medium text-stone-600 transition-all duration-100 hover:bg-stone-200 active:translate-y-0.5"
              >
                Open Drawer
              </Drawer.Trigger>
              <A
                href="/docs"
                class="py-3 w-fit px-4 font-medium text-lg text-stone-600"
              >
                Docs {"   ->"}
              </A>
            </div>
          ...
          <Drawer.Portal contextId="xxx">
            ...
            <Drawer.Content
              contextId="xxx"
              class="max-h-[90vh]  -bottom-5 w-full  rounded-b-lg bg-stone-50 "
            >
              <div class="h-1 w-10 self-center rounded-full bg-stone-300" />
              <div class="mt-3 grow divide-y divide-stone-400 overflow-y-auto" style={{"height": "var(--open-percentage)px"}}>
                <For each={new Array(20)}>
                  {(_, idx) => (
                    <p
                      class={clsx("py-2 text-center font-bold", {
                        "bg-stone-800": idx() % 2 === 0,
                      })}
                    >
                      List item {idx() + 1}
                    </p>
